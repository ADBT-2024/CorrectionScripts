...
async function computeOrderPrice (orderData) {
  let computedOrderPrice = orderData.products.reduce((total, product) => total + product.unityPrice * product.quantity, 0)
...
async function checkOrderEqualsOrderData (createdOrder, orderData) {
  ...
  for (const product of createdOrder.products) {
    ...
    expect(product.quantity).toEqual(matchingProduct.quantity)
    const fullProduct = await request(app).get(`/products/${product.id}`).send()
    expect(product.unityPrice).toEqual(fullProduct.body.price)
  }
}
